(function()
{
	...
	function toggle_breadcrumbs()
	{
		...
		function toggle_breadcrumbs_inner(breadcrumbs_container)
		{
			// Show breadcrumbs for the active tab, as well as breadcrumbs without a data-tab attr:
			$(`${container_element} ${breadcrumbs_container} > *[data-tab=${tab}], ${container_element} ${breadcrumbs_container} > *:not([data-tab]), ${container_element} ${breadcrumbs_container} > section > *[data-tab=${tab}], ${container_element} ${breadcrumbs_container} > section > *:not([data-tab])`).show().each(function()
			{
			...
			$(`${container_element} ${breadcrumbs_container} > *[data-tab]:not([data-tab=${tab}]) *, ${container_element} ${breadcrumbs_container} > section > *[data-tab]:not([data-tab=${tab}])`).hide();
		}

		// Process all breadcrumbs containers
		toggle_breadcrumbs_inner("#breadcrumbs");
		toggle_breadcrumbs_inner("#favorite-crumbs");
	}

	// Global values, available in external modules (i.e. crumb files)
	window.breadcrumbs =
	{
	...
	function breadcrumbs_init()
	{
		...
		function load_html()
		{
			...
			if (config.breadcrumbs_show)
			{
				...
				$("#breadcrumbs").before("<section id='favorite-crumbs'></section>");

				function load_crumbs(crumb_names, breadcrumbs_container = `${container_element} #breadcrumbs`)
				{
					debug(`Loading crumb layout files... ${crumb_names}`);
					var deferreds = [];

					$.each(crumb_names, function(index, value)
					{
						var deferred = $.Deferred();
						deferreds.push(deferred);

						// Set a unique global variable for this crumb file
						window[`breadcrumbs_container_${value}`] = breadcrumbs_container;
						...
						if (value == "extensions")
						{
							...
							insert_at(`${breadcrumbs_container}`, index, extensions_wrapper);
						...
						else
						{
							...
							{
								debug(`Loaded \`${value}\` crumb.`);
								// Add a button for each crumb
								var crumb_type = module.crumb_type ? module.crumb_type : "button";
								if (crumb_type == "button")
								{
									var crumb = add_breadcrumb(module.crumb_title, index, window[`breadcrumbs_container_${value}`]);
					...
					$.when.apply($, deferreds).done(function()
					{
						...
						$(`${breadcrumbs_container} > *`).detach();

						// Append the sorted breadcrumb elements back to the DOM
						$breadcrumbs.appendTo(`${breadcrumbs_container}`);

						toggle_breadcrumbs();
					});
				}

				// Load the crumb layout files, as well as favorite crumbs
				load_crumbs(config.breadcrumbs_crumb_layout);
				load_crumbs(config.breadcrumbs_favorite_crumbs, `${container_element} #favorite-crumbs`);
